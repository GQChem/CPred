...
def _di_prop(tables: PropensityTables, di_table: str,
             c1: str | None, c2: str | None) -> float:
    """Look up di-code propensity from a dedicated di-SS table."""
    if c1 is None or c2 is None:
        return 0.0
    return tables.get(di_table, c1 + c2)
...
def _extract_coupled_ss_features(
    ...
    for i in range(n):
        ...
        r2_sse[i] = _di_prop(tables, di_table_name, c, cp1)
        rxr_sse[i] = _di_prop(tables, di_table_name, cm1, cp1)
        r2xr_sse[i] = _di_prop(tables, di_table_name, cm2, cp1)
        r3xr_sse[i] = _di_prop(tables, di_table_name, cm3, cp1)
...
def extract_secondary_structure_features(
    ...
    features = {}
    features.update(_extract_coupled_ss_features(
        dssp_codes, tables, "dssp", "di_dssp", "sse"))
    features.update(_extract_coupled_ss_features(
        rama_codes, tables, "ramachandran", "di_ramachandran", "rm"))
    features.update(_extract_coupled_ss_features(
        ka_codes, tables, "kappa_alpha", "di_kappa_alpha", "ka"))
